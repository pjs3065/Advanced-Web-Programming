...
public class Hello3 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 검색어 입력 (단, 영어만)
		out.println("<div align='center'>" + "<form action='Hello3'>"
				+ "<h4>Choose a search engine and enter query keywords.</h4>" +
				// 선택
				"<select name='select' style='height:16pt'>" + "<option value='naver'>Naver</option>"
				+ "<option value='daum'>Daum</option>" + "<option value='google'>Google</option>"
				+ "<option value='yahoo'>Yahoo</option>" + "<option value='bing'>Bing</option>" + "</select>&nbsp;" +

				// 입력
				"<input type='text' name='query' value='' style='width:240px'/>"
				+ "<p><input type='submit' value='Search'/></form></div>" + "</body></html>");
	}
